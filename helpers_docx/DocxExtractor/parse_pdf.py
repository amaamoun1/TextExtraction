...
def parse_pdf(doc_id, pdf_path, pdfReaders, tables_to_parse, clean_obs):
    #set up document level data storage
    starting_dict={'doc_id':[doc_id], 'doc_path':[pdf_path]}
    doc_data = pd.DataFrame(starting_dict)
    
    table_id = 0
    unparsed = 0
    table_tracker = pd.DataFrame(columns=['doc_id', 'table', 'parsed'])
    tables_seen=[""]
    table_dict = {}
    
    #check the string for competency sections...this is the Morten - Like method
    comp_found = re.search("Competency Scorecard", doc_string, re.IGNORECASE | re.DOTALL)
    if comp_found == None:
        comp_found = re.search("Competencies.{0,1000}rating\s+and\s+comments", doc_string, re.IGNORECASE | re.DOTALL)
    if comp_found == None:
        comp_found = re.search("Competencies.{0,10000}removes\s+underperformers", doc_string, re.IGNORECASE | re.DOTALL)
    comp_found = int(comp_found!=None)
    ...
    for table_type in table_dict.keys():
        ...
        table_tracker = table_tracker.append({'doc_id':doc_id, 'table': table_type, 'parsed': parsed, 'issue': issue, 'unclean': is_unclean, 'string_parsed':string_parsed}, ignore_index=True, sort=True)
    ...
    for tab in tables_to_parse:
        if sum(table_tracker["table"].str.contains(tab))==0:
            ...
            if tab=="competency":
                string_parsed = comp_found
            table_tracker = table_tracker.append({'doc_id':doc_id, 'table': tab, 'parsed':0, 'issue': np.nan, 'unclean': np.nan, 'string_parsed':string_parsed}, ignore_index=True, sort=True)
